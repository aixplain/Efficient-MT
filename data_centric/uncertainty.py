...
from data_centric.utils.selection import multi_argmax, shuffled_argmax

from typing import List, Sequence, Union

classifier_type = Union[AutoML, VWModel]
import pdb
...
def classifier_prediction(
    classifier: classifier_type, X: myInput, **classifier_prediction_kwargs
) -> np.ndarray:
    """
    ...
    try:
        class_predictions = classifier.predict(X, **classifier_prediction_kwargs)
...
def classifier_uncertainty(
    classifier: classifier_type, X: myInput, **predict_proba_kwargs
) -> np.ndarray:
    """
    Classification uncertainty of the classifier for the provided samples.
...
def prediction_sampling(
    classifier: classifier_type,
    X: myInput,
    n_instances: int = 1,
    random_tie_break: bool = False,
    **classifier_prediction_kwargs
) -> np.ndarray:
    """
    ...
    """
    precitions = classifier_prediction(classifier, X, **classifier_prediction_kwargs)

    if not random_tie_break:
        return multi_argmax(precitions, n_instances=n_instances)

    return shuffled_argmax(precitions, n_instances=n_instances)
...
def regression_uncertainity_combined(
    classifier: classifier_type,
    X: myInput,
    n_instances: int = 1,
    random_tie_break: bool = False,
    **kwargs
    ...
    Returns:
        ...
        the instances from X chosen to be labelled.
    """
    precitions = classifier_prediction(classifier, X, **kwargs)
    uncertainty = classifier_uncertainty(classifier, X, **kwargs)

    combined = np.multiply(uncertainty, precitions)
    if not random_tie_break:
        return multi_argmax(combined, n_instances=n_instances)

    return shuffled_argmax(combined, n_instances=n_instances)
...
def regression_entropy_combined(
    classifier: classifier_type,
    X: myInput,
    n_instances: int = 1,
    random_tie_break: bool = False,
    **kwargs
    ...
    """
    precitions = classifier_prediction(classifier, X, **kwargs)
    entropy = classifier_entropy(classifier, X, **kwargs)

    combined = np.multiply(entropy, precitions)
    if not random_tie_break:
        return multi_argmax(combined, n_instances=n_instances)
